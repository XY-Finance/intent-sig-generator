...
def plot_interaction_types(hierarchical_metrics, base_path, qualitative_cmap, logger):
    ...
    for cluster, metrics in hierarchical_metrics.items():
        interaction_types_data[cluster] = {
            interaction_type: float(
                metrics["interaction_types"][interaction_type]["mean"]
            )
            for interaction_type in metrics["interaction_types"]
        }
    ...
    data = np.array(
        [
            [
                float(interaction_types_data[cluster][interaction_type])
    ...
    try:
        ...
        logger.info(" - interaction types plot saved.")
