...
def plot_cluster_metrics(hierarchical_metrics, base_path, qualitative_cmap, logger):
    """
    Plots and saves the repartition rates and address counts for clusters.
    """
    clusters = list(hierarchical_metrics.keys())
    repartition_rates = [
        float(hierarchical_metrics[cluster]["repartition_rate"]) for cluster in clusters
    ]
    address_count = [
        int(hierarchical_metrics[cluster]["address"]) for cluster in clusters
    ]
    ...
    try:
        ...
        logger.info(" - cluster repartition plot saved.")
